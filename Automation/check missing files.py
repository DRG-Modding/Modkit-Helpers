...
generated_files = 'F:\DRG Modding\Project Generator\FSDTemplateU38P7\Content'
...
def main():
    ...
    with open('missing_files_serialized.txt', 'w') as f:
        for root, _, files in os.walk(serialized_files):
            for file in files:
                if not file.endswith('.json'): continue
                if file.replace('.json', '') not in file_list:
                    print(f'File does not exist: {os.path.join(root, file.replace(".json", ".uasset"))}')
